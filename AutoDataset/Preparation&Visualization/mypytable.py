...
# 
# Description: This is the completed mypytable.py assignment from PA2
##############################################
...
class MyPyTable:
    ...
    def pretty_print(self, num_rows_shown=None):
        """Prints the table in a nicely formatted grid structure, 
        with an option to limit the number of rows displayed."""
    
        # checks if num_rows_shown parameter was provided
        if num_rows_shown is not None:
            data_to_print = self.data[:num_rows_shown] # this slices list (starts from beginning to end of specified row limit)
        
        # default is we display all rows
        else:
            data_to_print = self.data
    
        # Print the sliced or full data without modifying the actual dataset
        print(tabulate(data_to_print, headers=self.column_names))
    
        # checks if row limit was provided (not None) and indicates that there are more
        # rows that were't displayed
        if num_rows_shown is not None and len(self.data) > num_rows_shown:
            rest_of_data = len(self.data) - num_rows_shown
            print("...and ", rest_of_data, "more rows.")
