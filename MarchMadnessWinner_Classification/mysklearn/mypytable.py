...
# Description: This program contains various functions
# to help work with datasets
##############################################
...
class MyPyTable:
    ...
    def drop_columns(self, col_indexes_to_drop):
        """Remove columns from the table data.

        Args:
            col_indexes_to_drop(list of int): list of column indexes to remove from the table data.
        """
        # should sort the column indexes to drop in descending order to avoid index shifting issues
        col_indexes_to_drop.sort(reverse = True)
        cleaned_data = []  # Initialize an empty cleaned data set

        for row in self.data:  # Loop through each row
            cleaned_row = []  # Create a new list to store the cleaned row
            for i, value in enumerate(row):  # Enumerate over the row to get both index and value
                if i not in col_indexes_to_drop:  # If the column index is not in the drop list
                    cleaned_row.append(value)  # Add the value to the cleaned row
            cleaned_data.append(cleaned_row)  # Add the cleaned row to the cleaned data

        self.data = cleaned_data  # Update the data with the cleaned data

        updated_column_names = []

        for i, name in enumerate(self.column_names):
            if i not in col_indexes_to_drop:
                updated_column_names.append(name)
        
        self.column_names = updated_column_names
